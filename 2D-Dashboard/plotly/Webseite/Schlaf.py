...
@author: Mues
Die Klasse repräsentiert die Plotly-Version der Webseite des Schlafzimmers
"""
...
class Schlaf:
    ...
    def schlaf_seite(self, uhrzeiten, dashboard_datum_liste, dashboard_uhrzeit):
            ...
            if(dashboard_datum_liste != None and dashboard_uhrzeit != None):                               
                ...
                durchschnitt_temp = np.array(result_durchschnitt['t9']).mean().round(2)
                durchschnitt_luftfeucht = np.array(result_durchschnitt['rh_9']).mean().round(2)
